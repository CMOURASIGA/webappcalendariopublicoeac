...
const AgendaMensalShareEnhanced: React.FC<Props> = ({ mes, ano, eventos }) => {
  ...
  const densityMode: 'few' | 'medium' | 'many' = totalEvents >= 7 ? 'many' : totalEvents >= 4 ? 'medium' : 'few';
    ...
    >
      ...
      <main className={`px-14 pb-[300px] ${mainTopClass}`}>
        ...
        <div className="mb-6 flex gap-3">
          <span className="rounded-full bg-[#cf1526] px-5 py-2 text-[14px] font-black uppercase tracking-[0.08em] text-white">
            Vermelho = compromisso EAC
          </span>
          <span className="rounded-full bg-[#09325C] px-5 py-2 text-[14px] font-black uppercase tracking-[0.08em] text-white">
            Azul = data liturgica
          </span>
        </div>
              ...
              >
                <div className="flex gap-5">
                  ...
                  <div className={contentTopClass}>
                    ...
                    <p className="mt-1 text-[15px] font-black uppercase tracking-[0.16em] text-slate-400">
                      {evento.diaSemana.replace('.', '')}
                    </p>
                    <p className={timeClass}>
                      {evento.horario || 'Sem horario definido'}
                    </p>
                    <span
                      className={badgeClass}
                      style={{
                        backgroundColor: evento.contexto === 'eac' ? EAC_COLORS.accent : EAC_COLORS.primaryDark,
                        color: '#FFFFFF',
                      }}
                    >
                      {evento.contexto === 'eac' ? truncate(evento.tipo, 24) : 'Data liturgica'}
                    </span>
                  </div>
                </div>
              </article>
