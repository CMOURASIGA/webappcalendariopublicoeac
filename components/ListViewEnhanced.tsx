...
import { CalendarEvent, EVENT_COLORS, EVENT_TYPE_LABELS, getEventContextLabel } from '../types';
...
const ListViewEnhanced: React.FC<ListViewProps> = ({ events, onEventDetailsClick }) => {
  ...
  if (events.length === 0) {
    return (
      <div className="bg-white rounded-[40px] p-24 text-center border border-slate-100 shadow-xl">
        ...
        <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-xs">Agenda vazia para este periodo</p>
      </div>
    );
  }
          ...
          <div key={dateStr} className="flex flex-col md:flex-row gap-6 md:gap-10 group">
            ...
            <div className="flex-1 space-y-5">
              {grouped[dateStr].map((event) => {
                const contextLabel = getEventContextLabel(event.type);
                const timeLabel = event.startTime ? `${event.startTime}${event.endTime ? ` - ${event.endTime}` : ''}` : 'Sem horario definido';
                ...
                return (
                  <div key={event.id} className="bg-white p-6 md:p-10 rounded-[40px] shadow-sm border border-slate-50 flex flex-col md:flex-row md:items-center justify-between transition-all duration-300 hover:shadow-2xl hover:border-blue-100 hover:bg-white active:scale-[0.99]">
                    <div className="flex items-center gap-6">
                      <div className="hidden md:flex flex-col items-center justify-center text-[#112760] font-black text-sm w-24 border-r border-slate-100 pr-6 text-center">
                        {event.startTime || 'A definir'}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                          ...
                          <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg border ${getStatusClasses(event.status)}`}>
                            Situacao: {getStatusLabel(event.status)}
                          </span>
                          <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg border ${
                            event.startTime ? 'bg-blue-50 text-[#112760] border-blue-100' : 'bg-amber-50 text-amber-800 border-amber-200'
                          }`}>
                            {timeLabel}
                          </span>
                          <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg border ${
                            contextLabel === 'Compromisso EAC'
                              ? 'bg-[#cf1526]/10 text-[#a61120] border-[#cf1526]/15'
...
export default ListViewEnhanced;
